...
\( d(\cdot, \cdot) \) is the Euclidean distance, \( f_c = f_b (c + 1) \) is the class-specific frequency (\( f_b \): base frequency), \( \phi \) is phase, \( \gamma \) is damping strength, and \( p \) defines decay type (e.g., 2 for Gaussian). The predicted class is \( \arg\max_c E_c(\mathbf{x}) \).
